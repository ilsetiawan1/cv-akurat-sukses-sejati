// components/transaksi/GoodsReceiptTable.tsx
'use client';
...
export default function GoodsReceiptTable({ 
  data, total, currentPage, limit, searchQuery, items, suppliers, canCreate 
}: Props) {
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-[#F8F9FA] text-[#8E95A9] text-xs font-semibold border-b border-gray-100 uppercase tracking-wider">
                ...
                <th className="py-4 px-5 text-right">Total Nilai</th>
                ...
                <tr>
                  <td colSpan={7} className="p-12 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="font-medium text-gray-900">Belum ada barang masuk</p>
                      <p className="text-gray-400 text-xs mt-1">Data penerimaan stok akan muncul di sini.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                data.map((row) => (
                  <tr key={row.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="py-4 px-5 text-gray-600">
                      {new Date(row.receipt_date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </td>
                    ...
                    <td className="py-4 px-5 text-center font-bold text-[#7C3AED]">
                      +{row.quantity}
                    </td>
