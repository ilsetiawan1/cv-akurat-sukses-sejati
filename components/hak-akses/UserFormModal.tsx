...
const CRUD = ['can_create', 'can_read', 'can_update', 'can_delete'] as const;
...
function initPermissions(user?: UserWithPermissions | null): Record<FeatureKey, Record<string, boolean>> {
  // 1. Inisialisasi dengan tipe yang tepat
  const defaults = {} as Record<FeatureKey, Record<string, boolean>>;

  // 2. Isi default values secara manual agar TS yakin semua key terpenuhi
  ...
  user.user_permissions.forEach((p) => {
    const featureName = p.feature as FeatureKey;
    
    if (defaults[featureName]) {
...
export function UserFormModal({ open, user, onClose, onSuccess }: UserFormModalProps) {
  ...
  const [permissions, setPermissions] = useState(initPermissions(user));
  ...
  function togglePerm(feature: FeatureKey, field: string) {
    setPermissions((prev) => ({
      ...prev,
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!isEdit) {
        ...
        if (!result.success) { setError(result.error); setIsPending(false); return; }
      ...
      if (!permResult.success) { setError(permResult.error); setIsPending(false); return; }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input
                name="name" type="text" required defaultValue={user?.name}
            ...
            <div>
              ...
              <select
                name="role" defaultValue={user?.role ?? 'admin'}
            ...
            {isEdit && (
              <div>
                ...
                <select
                  name="status" defaultValue={user?.status ?? 'active'}
                  className="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all bg-white"
                >
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    {CRUD.map((c) => (
                      <th key={c} className="text-center px-3 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wide w-20">
                        {CRUD_LABELS[c]}
                      </th>
                    ))}
                  </tr>
                    ...
                    <tr key={key} className="hover:bg-gray-50 transition-colors">
                      ...
                      {CRUD.map((c) => (
                        <td key={c} className="px-3 py-3 text-center">
                          <input
                            type="checkbox"
                            checked={permissions[key][c] ?? false}
