// components/hak-akses/UserTable.tsx
...
function Avatar({ name, avatarUrl }: { name: string; avatarUrl: string | null }) {
  ...
  return <div className="w-9 h-9 rounded-full bg-purple-100 text-purple-600 text-sm font-bold flex items-center justify-center uppercase select-none">{name.charAt(0)}</div>;
}

function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${status === 'active' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
      {status === 'active' ? 'Active' : 'Inactive'}
    </span>
  );
}

// Skeleton dirender di server & client identik → tidak ada hydration mismatch
...
export function UserTable() {
  ...
  // FIX HYDRATION: gunakan suppressHydrationWarning + initialLoading terpisah
  // Server render: showSkeleton=true (hardcoded, konsisten)
  // Client: fetch data, lalu showSkeleton=false
  ...
  const loadUsers = useCallback((p: number, q: string) => {
    startTransition(async () => {
      const result = await getUsersAction(p, LIMIT, q);
  ...
  // Initial load — hanya di client setelah mount
  useEffect(() => {
    loadUsers(1, '');
  }, [loadUsers]);

  // Re-fetch saat page / search berubah (skip initial karena loadUsers sudah jalan)
  useEffect(() => {
    if (!showSkeleton) loadUsers(page, search);
  ...
  function handleEdit(user: UserWithPermissions) {
    setEditUser(user);
    setModalOpen(true);
  }
  ...
  function handleExport() {
    ...
    a.href = url;
    a.download = 'hak-akses-pengguna.csv';
    a.click();
    URL.revokeObjectURL(url);
  }

  const isLoading = showSkeleton || isPending;
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            {/* suppressHydrationWarning: tbody konten berbeda server vs client (skeleton vs data) */}
            <tbody
              ...
              {isLoading ? (
                ...
                users.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <span
                        className={`text-xs font-medium px-2.5 py-1 rounded-full ${user.role === 'super_admin' ? 'bg-purple-50 text-purple-700' : 'bg-gray-100 text-gray-600'}`}
                      >
